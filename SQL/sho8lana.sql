...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 13, 2019 at 03:55 PM
...
--
-- Structure for view `palance`
--
DROP TABLE IF EXISTS `palance`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `palance`  AS  select sum(`offers`.`PriceOffers`) AS `total`,`users`.`User_id` AS `User_id`,`services`.`service_title` AS `service_title`,`services`.`services_stat` AS `services_stat` from ((`users` join `offers`) join `services`) where ((`offers`.`OffersUser` = `users`.`User_id`) and (`offers`.`OffersService` = `services`.`id_services`) and (`offers`.`OffersStat` = 2)) group by `offers`.`PriceOffers`,`users`.`User_id`,`services`.`service_title`,`services`.`services_stat` ;
